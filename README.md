...
- **Outcome classification**: byte counts and returned values carry progress, while semantic errors carry control.
  Direct `sock` calls usually surface `iox.ErrWouldBlock` on not-ready paths and `ErrInProgress` on pending connect;
  `iox.ErrMore`, `Classify`, `IsSemantic`, and `IsProgress` remain shared classifier vocabulary for helpers layered
  above `sock`.
